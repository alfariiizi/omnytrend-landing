        ...
        {items !== false && items.length > 0 && (
          <div className="grid grid-cols-2 gap-12 sm:grid-cols-4">
            {items.map((item, index) => (
              <div
                key={index}
                className="group flex cursor-default flex-col items-start gap-3 text-left"
              >
                {item.label && (
                ...
                <div className="flex items-baseline gap-2">
                  <div className="inline-block text-4xl font-bold text-white transition-transform duration-300 group-hover:scale-110">
                    {item.value}
                  </div>
                  {item.suffix && (
                    <div
                      className={`${item.color} text-brand ml-0.5 text-2xl font-semibold`}
                    >
                      {item.suffix}
                    </div>
                  )}
